    ...
    <header className={`h-14 border-b px-4 flex items-center justify-between text-xs select-none sticky top-0 z-50 transition-colors ${
      ...
      <div className="flex items-center gap-3">
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
          isLight
            ...
            : 'bg-emerald-950/80 text-emerald-400 border border-emerald-500/40 shadow-[0_0_14px_rgba(16,185,129,0.3)]'
        }`}>
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className={`px-1.5 py-0.5 rounded text-[10px] font-mono font-medium border ${
              ...
              {isLight ? 'DAY MODE' : 'NIGHT MODE (GREEN)'}
            </span>
          </div>
      ...
      <div className={`hidden md:flex items-center gap-2 px-3 py-1 rounded-full border ${
        isLight
          ...
          : 'bg-[#111622] border-[#232d42] text-emerald-400'
      }`}>
        <span className="relative flex h-2 w-2">
          <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${
            isLight ? 'bg-slate-700' : 'bg-emerald-400'
          }`} />
          <span className={`relative inline-flex rounded-full h-2 w-2 ${
            isLight ? 'bg-slate-900' : 'bg-emerald-500'
          }`} />
        </span>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        <div className={`hidden lg:flex items-center gap-1.5 px-2.5 py-1 rounded border font-mono text-[11px] ${
          ...
          <Activity className={`w-3 h-3 ${isLight ? 'text-slate-900' : 'text-emerald-400'}`} />
        ...
        <div className={`hidden lg:flex items-center gap-1.5 px-2.5 py-1 rounded border font-mono text-[11px] ${
          ...
          <Server className={`w-3 h-3 ${isLight ? 'text-slate-900' : 'text-emerald-400'}`} />
        ...
        <div className={`hidden lg:flex items-center gap-1.5 px-2.5 py-1 rounded border font-mono text-[11px] ${
          ...
          <Database className={`w-3 h-3 ${isLight ? 'text-slate-900' : 'text-emerald-400'}`} />
        ...
        <button
          ...
          className={`p-2 rounded-lg border font-mono flex items-center gap-1.5 text-xs transition-all ${
            isLight
              ...
              : 'bg-[#111622] hover:bg-[#1a2234] border-[#232d42] text-emerald-400'
          }`}
          title={isLight ? 'Switch to Night Mode (Green/Black)' : 'Switch to Day Mode (Grey/White/Black)'}
        >
          {isLight ? <Moon className="w-4 h-4 text-slate-800" /> : <Sun className="w-4 h-4 text-emerald-400" />}
