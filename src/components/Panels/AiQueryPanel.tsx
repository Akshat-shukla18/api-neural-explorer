...
export const AiQueryPanel: React.FC<AiQueryPanelProps> = ({
  ...
  return (
    <div className={`w-full h-full flex flex-col font-sans select-none overflow-hidden text-xs transition-colors ${
      ...
      <div className={`px-4 py-2 border-b font-mono text-[11px] ${
          ...
          <div className={`p-3 rounded-lg border space-y-2 ${
            ...
            <div className="flex flex-col gap-1.5">
              {SAMPLE_QUESTIONS.map((q, idx) => (
                ...
                >
                  <span>{q}</span>
                  <ArrowRight className="w-3 h-3 group-hover:translate-x-0.5 transition-transform opacity-70" />
                </button>
              ))}
            </div>
          </div>
